...
export async function GET(request) {
  ...
  if (searchParams.get('today') == "true") {
    let json = JSON.parse(fs.readFileSync(directory + "/" +'volume_bydate.json', 'utf-8'));
    let volumeToday;
  ...
  if (searchParams.get('sort') == "strategy") {
    // let data = JSON.parse(fs.readFileSync(directory + "/" +'volume_bystrategy.json', 'utf-8'));
    // return NextResponse.json({ data }, { status: 200 });
  } else if (searchParams.get('sort') == "date") {
    let data = JSON.parse(fs.readFileSync(directory + "/" +'volume_bydate.json', 'utf-8'));
    return NextResponse.json({ data }, { status: 200 });
  }
  ...
  fs.writeFileSync(directory + "/" +'volume_bydate.json', JSON.stringify(volumeByDate));
